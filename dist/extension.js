...
`):`Error: ${l.message||l}`}var Re,pe,ae=Y(()=>{"use strict";Re=$(require("vscode"));_e();Ae();try{pe=require("odbc")}catch{console.error("odbc package not installed. Run: npm install odbc")}});var ot={};Q(ot,{SchemaItem:()=>j,SchemaProvider:()=>ge});var O,ge,j,Ue=Y(()=>{"use strict";O=$(require("vscode"));ae();ge=class{constructor(e,t){this.context=e;this.connectionManager=t;this._onDidChangeTreeData=new O.EventEmitter;this.onDidChangeTreeData=this._onDidChangeTreeData.event}refresh(){this._onDidChangeTreeData.fire()}getTreeItem(e){return e}getParent(e){if(e.contextValue!=="database"){if(e.contextValue==="typeGroup")return new j(e.dbName,O.TreeItemCollapsibleState.Collapsed,"database",e.dbName);if(e.contextValue.startsWith("netezza:"))return new j(e.objType,O.TreeItemCollapsibleState.Collapsed,"typeGroup",e.dbName,e.objType);if(e.contextValue==="column")return}}async getChildren(e){if(!await this.connectionManager.getConnectionString())return[];if(e){if(e.contextValue==="database")try{let s=`SELECT DISTINCT OBJTYPE FROM ${e.dbName}.._V_OBJECT_DATA WHERE DBNAME = '${e.dbName}' ORDER BY OBJTYPE`,n=await L(this.context,s,!0);return JSON.parse(n||"[]").map(o=>new j(o.OBJTYPE,O.TreeItemCollapsibleState.Collapsed,"typeGroup",e.dbName,o.OBJTYPE))}catch(s){return O.window.showErrorMessage("Failed to load object types: "+s),[]}else if(e.contextValue==="typeGroup")try{let s=`SELECT OBJNAME, SCHEMA, OBJID, COALESCE(DESCRIPTION, '') AS DESCRIPTION FROM ${e.dbName}.._V_OBJECT_DATA WHERE DBNAME = '${e.dbName}' AND OBJTYPE = '${e.objType}' ORDER BY OBJNAME`,n=await L(this.context,s,!0);return JSON.parse(n||"[]").map(o=>{let r=["TABLE","VIEW","EXTERNAL TABLE","SYSTEM VIEW","SYSTEM TABLE"].includes(e.objType||"");return new j(o.OBJNAME,r?O.TreeItemCollapsibleState.Collapsed:O.TreeItemCollapsibleState.None,`netezza:${e.objType}`,e.dbName,e.objType,o.SCHEMA,o.OBJID,o.DESCRIPTION)})}catch(s){return O.window.showErrorMessage("Failed to load objects: "+s),[]}else if(e.contextValue.startsWith("netezza:")&&e.objId)try{let s=`SELECT 
                    ...
                    WHERE
                        X.OBJID = ${e.objId}
                    ORDER BY 
                        X.ATTNUM`,n=await L(this.context,s,!0);return JSON.parse(n||"[]").map(o=>new j(`${o.ATTNAME} (${o.FORMAT_TYPE})`,O.TreeItemCollapsibleState.None,"column",e.dbName,void 0,void 0,void 0,o.DESCRIPTION))}catch(s){return O.window.showErrorMessage("Failed to load columns: "+s),[]}}else try{let s=await L(this.context,"SELECT DATABASE FROM system.._v_database ORDER BY DATABASE",!0);return s?JSON.parse(s).map(i=>new j(i.DATABASE,O.TreeItemCollapsibleState.Collapsed,"database",i.DATABASE)):(console.log("No results from database query"),[])}catch(s){return O.window.showErrorMessage("Failed to load databases: "+s),console.error(s),[]}return[]}},j=class extends O.TreeItem{constructor(t,s,n,i,o,a,r,p){super(t,s);this.label=t;this.collapsibleState=s;this.contextValue=n;this.dbName=i;this.objType=o;this.schema=a;this.objId=r;this.objectDescription=p;let u=this.label;p&&p.trim()&&(u+=`

${p.trim()}`),a&&n.startsWith("netezza:")&&(u+=`

Schema: ${a}`),this.tooltip=u,this.description=a?`(${a})`:"",n==="database"?this.iconPath=new O.ThemeIcon("database"):n==="typeGroup"?this.iconPath=new O.ThemeIcon("folder"):n.startsWith("netezza:")?this.iconPath=this.getIconForType(o):n==="column"&&(this.iconPath=new O.ThemeIcon("symbol-field"))}getIconForType(t){switch(t){case"TABLE":return new O.ThemeIcon("table");case"VIEW":return new O.ThemeIcon("eye");case"PROCEDURE":return new O.ThemeIcon("gear");case"FUNCTION":return new O.ThemeIcon("symbol-function");case"AGGREGATE":return new O.ThemeIcon("symbol-operator");case"EXTERNAL TABLE":return new O.ThemeIcon("server");default:return new O.ThemeIcon("file")}}}});var ct={};Q(ct,{generateDDL:()=>Bt});function P(l){return!l||/^[A-Z_][A-Z0-9_]*$/i.test(l)&&l===l.toUpperCase()?l:`"${l.replace(/"/g,'""')}"`}async function at(l,e,t,s){let n=`
        SELECT 
    ...
    `,i=await l.query(n),o=[];for(let a of i)o.push({name:a.ATTNAME,description:a.DESCRIPTION||null,fullTypeName:a.FULL_TYPE,notNull:!!a.ATTNOTNULL,defaultValue:a.COLDEFAULT||null});return o}async function Nt(l,e,t,s){try{let n=`
        ...
        `;return(await l.query(n)).map(o=>o.ATTNAME)}catch{return[]}}async function Dt(l,e,t,s){try{let n=`
            ...
            WHERE SCHEMA = '${t.toUpperCase()}'
                AND TABLENAME = '${s.toUpperCase()}'
            ORDER BY ORGSEQNO
        `;return(await l.query(n)).map(o=>o.ATTNAME)}catch{return[]}}async function xt(l,e,t,s){let n=`
        SELECT 
            X.SCHEMA
            , X.RELATION
            , X.CONSTRAINTNAME
    ...
    `,i=new Map;try{let o=await l.query(n);for(let a of o){let r=a.CONSTRAINTNAME;if(!i.has(r)){let u={p:"PRIMARY KEY",f:"FOREIGN KEY",u:"UNIQUE"};i.set(r,{type:u[a.CONTYPE]||"UNKNOWN",typeChar:a.CONTYPE,columns:[],pkDatabase:a.PKDATABASE||null,pkSchema:a.PKSCHEMA||null,pkRelation:a.PKRELATION||null,pkColumns:[],updateType:a.UPDT_TYPE||"NO ACTION",deleteType:a.DEL_TYPE||"NO ACTION"})}let p=i.get(r);p.columns.push(a.ATTNAME),a.PKATTNAME&&p.pkColumns.push(a.PKATTNAME)}}catch(o){console.warn("Cannot retrieve keys info:",o)}return i}async function Mt(l,e,t,s){try{let n=`
            SELECT DESCRIPTION
        ...
        `,i=await l.query(n);if(i.length>0&&i[0].DESCRIPTION)return i[0].DESCRIPTION}catch{try{let n=`
                ...
                WHERE SCHEMA = '${t.toUpperCase()}'
                    AND OBJNAME = '${s.toUpperCase()}'
            `,i=await l.query(n);if(i.length>0&&i[0].DESCRIPTION)return i[0].DESCRIPTION}catch{}}return null}async function Ot(l,e,t,s){let n=await at(l,e,t,s);if(n.length===0)throw new Error(`Table ${e}.${t}.${s} not found or has no columns`);let i=await Nt(l,e,t,s),o=await Dt(l,e,t,s),a=await xt(l,e,t,s),r=await Mt(l,e,t,s),p=P(e),u=P(t),m=P(s),f=[];f.push(`CREATE TABLE ${p}.${u}.${m}`),f.push("(");let g=[];for(let v of n){let R=`    ${P(v.name)} ${v.fullTypeName}`;v.defaultValue&&(R+=` DEFAULT ${v.defaultValue}`),g.push(R)}if(f.push(g.join(`,
`)),i.length>0){let v=i.map(S=>P(S));f.push(`)
DISTRIBUTE ON (${v.join(", ")})`)}else f.push(`)
DISTRIBUTE ON RANDOM`);if(o.length>0){let v=o.map(S=>P(S));f.push(`ORGANIZE ON (${v.join(", ")})`)}f.push(";"),f.push("");for(let[v,S]of a){let R=P(v),D=S.columns.map(N=>P(N));if(S.typeChar==="f"){let N=S.pkColumns.filter(x=>x).map(x=>P(x));N.length>0&&f.push(`ALTER TABLE ${p}.${u}.${m} ADD CONSTRAINT ${R} ${S.type} (${D.join(", ")}) REFERENCES ${S.pkDatabase}.${S.pkSchema}.${S.pkRelation} (${N.join(", ")}) ON DELETE ${S.deleteType} ON UPDATE ${S.updateType};`)}else(S.typeChar==="p"||S.typeChar==="u")&&f.push(`ALTER TABLE ${p}.${u}.${m} ADD CONSTRAINT ${R} ${S.type} (${D.join(", ")});`)}if(r){let v=r.replace(/'/g,"''");f.push(""),f.push(`COMMENT ON TABLE ${p}.${u}.${m} IS '${v}';`)}for(let v of n)if(v.description){let S=P(v.name),R=v.description.replace(/'/g,"''");f.push(`COMMENT ON COLUMN ${p}.${u}.${m}.${S} IS '${R}';`)}return f.join(`
...
`,objectInfo:{database:e,schema:t,objectName:s,objectType:n},note:`${n} DDL generation not yet implemented`}}catch(o){return{success:!1,error:`DDL generation error: ${o.message||o}`}}finally{if(i)try{await i.close()}catch{}}}var it,lt=Y(()=>{"use strict";it=$(require("odbc"))});var Ee={};Q(Ee,{copyFileToClipboard:()=>He,exportCsvToXlsb:()=>Ft,exportQueryToXlsb:()=>Ut,getTempFilePath:()=>Ht});async function Ut(l,e,t,s=!1,n){let i=null;try{n&&n("Connecting to database..."),i=await zt.connect(l),n&&n("Executing query...");let o=await i.query(e);if(!o||!o.columns||o.columns.length===0)return{success:!1,message:"Query did not return any results (no columns)"};let a=o.columns.length;n&&n(`Query returned ${a} columns`);let r=o.columns.map(x=>x.name),p=o.map(x=>r.map(H=>x[H])),u=p.length;n&&n(`Writing ${u.toLocaleString()} rows to XLSX: ${t}`);let m=k.utils.book_new(),f=[r,...p],g=k.utils.aoa_to_sheet(f);k.utils.book_append_sheet(m,g,"Query Results");let v=[["SQL Query:"],...e.split(`
`).map(x=>[x])],S=k.utils.aoa_to_sheet(v);k.utils.book_append_sheet(m,S,"SQL Code"),k.writeFile(m,t,{bookType:"xlsx",compression:!0});let D=Fe.statSync(t).size/(1024*1024);n&&(n("XLSX file created successfully"),n(`  - Rows: ${u.toLocaleString()}`),n(`  - Columns: ${a}`),n(`  - File size: ${D.toFixed(1)} MB`),n(`  - Location: ${t}`));let N={success:!0,message:`Successfully exported ${u} rows to ${t}`,details:{rows_exported:u,columns:a,file_size_mb:parseFloat(D.toFixed(1)),file_path:t}};if(s){n&&n("Copying to clipboard...");let x=await He(t);N.details&&(N.details.clipboard_success=x)}return N}catch(o){return{success:!1,message:`Export error: ${o.message||o}`}}finally{if(i)try{await i.close()}catch{}}}async function Ft(l,e,t=!1,s,n){try{n&&n("Reading CSV content...");let i=k.read(l,{type:"string",raw:!0});if(!i.SheetNames||i.SheetNames.length===0)return{success:!1,message:"CSV content is empty or invalid"};let o=i.Sheets[i.SheetNames[0]],a=k.utils.sheet_to_json(o,{header:1});if(a.length===0)return{success:!1,message:"CSV file is empty or contains no headers"};let r=a.length-1,p=a[0]?a[0].length:0;n&&n(`Writing ${r.toLocaleString()} rows to XLSX: ${e}`);let u=k.utils.book_new(),m=k.utils.aoa_to_sheet(a);k.utils.book_append_sheet(u,m,"CSV Data");let f=s?.source||"Clipboard",g=[["CSV Source:"],[f]],v=k.utils.aoa_to_sheet(g);k.utils.book_append_sheet(u,v,"CSV Source"),k.writeFile(u,e,{bookType:"xlsx",compression:!0});let R=Fe.statSync(e).size/(1024*1024);n&&(n("XLSX file created successfully"),n(`  - Rows: ${r.toLocaleString()}`),n(`  - Columns: ${p}`),n(`  - File size: ${R.toFixed(1)} MB`),n(`  - Location: ${e}`));let D={success:!0,message:`Successfully exported ${r} rows from CSV to ${e}`,details:{rows_exported:r,columns:p,file_size_mb:parseFloat(R.toFixed(1)),file_path:e}};if(t){n&&n("Copying to clipboard...");let N=await He(e);D.details&&(D.details.clipboard_success=N)}return D}catch(i){return{success:!1,message:`Export error: ${i.message||i}`}}}async function He(l){return Oe.platform()!=="win32"?(console.error("Clipboard file copy is only supported on Windows"),!1):new Promise(e=>{try{let t=le.normalize(le.resolve(l)),s=`Set-Clipboard -Path "${t.replace(/"/g,'`"')}"`,n=(0,dt.spawn)("powershell.exe",["-NoProfile","-NonInteractive","-Command",s]),i="";n.stderr.on("data",o=>{i+=o.toString()}),n.on("close",o=>{o!==0?(console.error(`PowerShell clipboard copy failed: ${i}`),e(!1)):(console.log(`File copied to clipboard: ${t}`),e(!0))}),n.on("error",o=>{console.error(`Error spawning PowerShell: ${o.message}`),e(!1)})}catch(t){console.error(`Error copying file to clipboard: ${t.message}`),e(!1)}})}function Ht(){let l=Oe.tmpdir(),t=`netezza_export_${Date.now()}.xlsx`;return le.join(l,t)}var k,Fe,le,Oe,dt,zt,we=Y(()=>{"use strict";k=$(require("xlsx")),Fe=$(require("fs")),le=$(require("path")),Oe=$(require("os")),dt=require("child_process"),zt=require("odbc")});var pt={};Q(pt,{exportToCsv:()=>kt});async function kt(l,e,t,s,n){if(!Xe)throw new Error("odbc package not installed. Please run: npm install odbc");let i=await Xe.connect(e);try{n&&n.report({message:"Executing query..."});let o=await i.query(t,{cursor:!0,fetchSize:1e3});n&&n.report({message:"Writing to CSV..."});let a=ut.createWriteStream(s,{encoding:"utf8",highWaterMark:64*1024}),r=[];o.columns&&(r=o.columns.map(g=>g.name)),r.length>0&&a.write(r.map(ke).join(",")+`
`);let p=0,u=[],m=[],f=100;do{u=await o.fetch();for(let g of u){p++;let v;if(r.length>0?v=r.map(S=>ke(g[S])):v=Object.values(g).map(S=>ke(S)),m.push(v.join(",")),m.length>=f){let S=a.write(m.join(`
`)+`
...
`)+`
`),await o.close(),a.end(),await new Promise((g,v)=>{a.on("finish",g),a.on("error",v)}),n&&n.report({message:`Completed: ${p} rows exported`})}finally{try{await i.close()}catch(o){console.error("Error closing connection:",o)}}}function ke(l){if(l==null)return"";let e="";return typeof l=="bigint"?l>=Number.MIN_SAFE_INTEGER&&l<=Number.MAX_SAFE_INTEGER?e=Number(l).toString():e=l.toString():l instanceof Date?e=l.toISOString():l instanceof Buffer?e=l.toString("hex"):typeof l=="object"?e=JSON.stringify(l):e=String(l),e.includes('"')||e.includes(",")||e.includes(`
`)||e.includes("\r")?`"${e.replace(/"/g,'""')}"`:e}var ut,Xe,gt=Y(()=>{"use strict";ut=$(require("fs"));try{Xe=require("odbc")}catch{console.error("odbc package not installed. Run: npm install odbc")}});var mt={};Q(mt,{ColumnTypeChooser:()=>de,NetezzaDataType:()=>G,NetezzaImporter:()=>Le,importDataToNetezza:()=>Xt});async function Xt(l,e,t,s,n){let i=Date.now();try{if(!l||!X.existsSync(l))return{success:!1,message:`Source file not found: ${l}`};if(!e)return{success:!1,message:"Target table name is required"};if(!t)return{success:!1,message:"Connection string is required"};let a=X.statSync(l).size,r=ee.extname(l).toLowerCase(),p=[".csv",".txt",".xlsx",".xlsb"];if(!p.includes(r))return{success:!1,message:`Unsupported file format: ${r}. Supported: ${p.join(", ")}`};if([".xlsx",".xlsb"].includes(r)&&!ve)return{success:!1,message:"XLSX module not available. Please run: npm install xlsx"};n?.("Starting import process..."),n?.(`  Source file: ${l}`),n?.(`  Target table: ${e}`),n?.(`  File size: ${a.toLocaleString()} bytes`),n?.(`  File format: ${r}`);let m=new Le(l,e,t);await m.analyzeDataTypes(n),n?.("Using file-based import...");let f=await m.createDataFile(n),g=m.generateCreateTableSql();if(n?.("Generated SQL:"),n?.(g),n?.("Connecting to Netezza..."),!qe)throw new Error("ODBC module not available");let v=await qe.connect(t);try{n?.("Executing CREATE TABLE with EXTERNAL data..."),await v.query(g),n?.("Import completed successfully")}finally{await v.close();try{X.existsSync(f)&&(X.unlinkSync(f),n?.("Temporary data file cleaned up"))}catch(R){n?.(`Warning: Could not clean up temp file: ${R.message}`)}}let S=(Date.now()-i)/1e3;return{success:!0,message:"Import completed successfully",details:{sourceFile:l,targetTable:e,fileSize:a,format:r,rowsProcessed:m.getRowsCount(),rowsInserted:m.getRowsCount(),processingTime:`${S.toFixed(1)}s`,columns:m.getSqlHeaders().length,detectedDelimiter:m.getCsvDelimiter()}}}catch(o){let a=(Date.now()-i)/1e3;return{success:!1,message:`Import failed: ${o.message}`,details:{processingTime:`${a.toFixed(1)}s`}}}}var X,ee,ve,qe,G,de,Le,We=Y(()=>{"use strict";X=$(require("fs")),ee=$(require("path"));try{ve=require("xlsx")}catch{console.error("XLSX module not available")}try{qe=require("odbc")}catch{console.error("ODBC module not available")}G=class{constructor(e,t,s,n){this.dbType=e;this.precision=t;this.scale=s;this.length=n}toString(){return["BIGINT","DATE","DATETIME"].includes(this.dbType)?this.dbType:this.dbType==="NUMERIC"?`${this.dbType}(${this.precision},${this.scale})`:this.dbType==="NVARCHAR"?`${this.dbType}(${this.length})`:`TODO !!! ${this.dbType}`}},de=class{constructor(){this.decimalDelimInCsv=".";this.firstTime=!0;this.currentType=new G("BIGINT")}getType(e){let t=this.currentType.dbType,s=e.length;if(t==="BIGINT"&&/^\d+$/.test(e)&&s<15)return this.firstTime=!1,new G("BIGINT");let n=(e.match(new RegExp(`\\${this.decimalDelimInCsv}`,"g"))||[]).length;if(["BIGINT","NUMERIC"].includes(t)&&n<=1){let o=e.replace(this.decimalDelimInCsv,"");if(/^\d+$/.test(o)&&s<15&&(!o.startsWith("0")||n>0))return this.firstTime=!1,new G("NUMERIC",16,6)}if((t==="DATE"||this.firstTime)&&(e.match(/-/g)||[]).length===2&&s>=8&&s<=10){let o=e.split("-");if(o.length===3&&o.every(a=>/^\d+$/.test(a)))try{let a=new Date(parseInt(o[0]),parseInt(o[1])-1,parseInt(o[2]));if(!isNaN(a.getTime()))return this.firstTime=!1,new G("DATE")}catch{}}if((t==="DATETIME"||this.firstTime)&&(e.match(/-/g)||[]).length===2&&s>=12&&s<=20){let o=e.match(/^(\d{4})-(\d{1,2})-(\d{1,2})[\s|T](\d{2}):(\d{2})(:?(\d{2}))?$/);if(o)try{let a=o[7]?parseInt(o[7]):0,r=new Date(parseInt(o[1]),parseInt(o[2])-1,parseInt(o[3]),parseInt(o[4]),parseInt(o[5]),a);if(!isNaN(r.getTime()))return this.firstTime=!1,new G("DATETIME")}catch{}}let i=Math.max(s+5,20);return this.currentType.length!==void 0&&i<this.currentType.length&&(i=this.currentType.length),this.firstTime=!1,new G("NVARCHAR",void 0,void 0,i)}refreshCurrentType(e){return this.currentType=this.getType(e),this.currentType}},Le=class{constructor(e,t,s,n){this.delimiter="	";this.delimiterPlain="\\t";this.recordDelim=`
...
) DISTRIBUTE ON RANDOM;`}async createDataFile(e){let t=ee.join(this.logDir,`netezza_import_data_${Math.floor(Math.random()*1e3)}.txt`);e?.(`Creating temporary data file: ${t}`);try{let s;if(this.isExcelFile){if(!this.excelData||this.excelData.length===0)throw new Error("Excel data not loaded. Call analyzeDataTypes first.");s=this.excelData.slice(1)}else{let i=X.readFileSync(this.filePath,"utf-8");i.startsWith("\uFEFF")&&(i=i.slice(1));let o=i.split(/\r?\n/);s=[];let a=!0;for(let r of o)if(r.trim()){if(a){a=!1;continue}s.push(this.parseCsvLine(r))}}let n=[];for(let i=0;i<s.length;i++){let a=s[i].map((r,p)=>this.formatValue(r||"",p));n.push(a.join(this.delimiter)),(i+1)%1e4===0&&e?.(`Processed ${(i+1).toLocaleString()} rows...`)}return X.writeFileSync(t,n.join(this.recordDelim),"utf-8"),this.pipeName=t.replace(/\\/g,"/"),e?.(`Data file created: ${this.pipeName}`),t}catch(s){throw new Error(`Error creating data file: ${s.message}`)}}getRowsCount(){return this.rowsCount}getSqlHeaders(){return this.sqlHeaders}getCsvDelimiter(){return this.csvDelimiter}}});var ft={};Q(ft,{ClipboardDataProcessor:()=>$e,importClipboardDataToNetezza:()=>Jt});function qt(l){let e=String(l).trim();return e=e.replace(/[^0-9a-zA-Z]+/g,"_").toUpperCase(),(!e||/^\d/.test(e))&&(e="COL_"+e),e}function Wt(l,e,t){let s=String(l).trim();for(let n of t)s=s.split(n).join(`${e}${n}`);return s}function Vt(l,e,t,s,n){let i=Wt(l,s,n);return e<t.length&&t[e].currentType.dbType==="DATETIME"&&(i=i.replace("T"," ")),i}async function Jt(l,e,t,s,n){let i=Date.now(),o=null;try{if(!l)return{success:!1,message:"Target table name is required"};if(!e)return{success:!1,message:"Connection string is required"};n?.("Starting clipboard import process..."),n?.(`  Target table: ${l}`),n?.(`  Format preference: ${t||"auto-detect"}`);let a=new $e,[r,p]=await a.processClipboardData(t,n);if(!r||!r.length)return{success:!1,message:"No data found in clipboard"};if(r.length<2)return{success:!1,message:"Clipboard data must contain at least headers and one data row"};n?.(`  Detected format: ${p}`),n?.(`  Rows: ${r.length}`),n?.(`  Columns: ${r[0].length}`);let u=r[0].map(B=>qt(B)),m=r.slice(1);n?.("Analyzing clipboard data types...");let f=u.map(()=>new de);for(let B=0;B<m.length;B++){let ne=m[B];for(let d=0;d<ne.length;d++)d<f.length&&ne[d]&&ne[d].trim()&&f[d].refreshCurrentType(ne[d].trim());(B+1)%1e3===0&&n?.(`Analyzed ${(B+1).toLocaleString()} rows...`)}n?.(`Analysis complete: ${m.length.toLocaleString()} data rows`);let g=Ve.join(require("os").tmpdir(),"netezza_clipboard_logs");K.existsSync(g)||K.mkdirSync(g,{recursive:!0});let v="	",S="\\t",R=`
`,D="\\n",N="\\",x=[N,R,"\r",v];o=Ve.join(g,`netezza_clipboard_import_${Math.floor(Math.random()*1e3)}.txt`),n?.(`Creating temporary data file: ${o}`);let H=[];for(let B=0;B<m.length;B++){let d=m[B].map((h,w)=>Vt(h,w,f,N,x));H.push(d.join(v)),(B+1)%1e3===0&&n?.(`Processed ${(B+1).toLocaleString()} rows...`)}K.writeFileSync(o,H.join(R),"utf-8");let te=o.replace(/\\/g,"/");n?.(`Data file created: ${te}`);let ye=[];for(let B=0;B<u.length;B++)ye.push(`        ${u[B]} ${f[B].currentType.toString()}`);let Pe=g.replace(/\\/g,"/"),Te=`CREATE TABLE ${l} AS 
(
...
) DISTRIBUTE ON RANDOM;`;if(n?.("Generated SQL:"),n?.(Te),n?.("Connecting to Netezza..."),!Je)throw new Error("ODBC module not available");let be=await Je.connect(e);try{n?.("Executing CREATE TABLE with EXTERNAL clipboard data..."),await be.query(Te),n?.("Clipboard import completed successfully")}finally{await be.close()}let Ye=(Date.now()-i)/1e3;return{success:!0,message:"Clipboard import completed successfully",details:{targetTable:l,format:p,rowsProcessed:m.length,rowsInserted:m.length,processingTime:`${Ye.toFixed(1)}s`,columns:u.length,detectedDelimiter:v}}}catch(a){let r=(Date.now()-i)/1e3;return{success:!1,message:`Clipboard import failed: ${a.message}`,details:{processingTime:`${r.toFixed(1)}s`}}}finally{if(o&&K.existsSync(o))try{K.unlinkSync(o),n?.("Temporary clipboard data file cleaned up")}catch(a){n?.(`Warning: Could not clean up temp file: ${a.message}`)}}}var K,Ve,ht,Je,$e,Et=Y(()=>{"use strict";K=$(require("fs")),Ve=$(require("path")),ht=$(require("vscode"));We();try{Je=require("odbc")}catch{console.error("ODBC module not available")}$e=class{constructor(){this.processedData=[]}processXmlSpreadsheet(e,t){t?.("Processing XML Spreadsheet data...");let s=[],n=0,i=[],o=0,a=e.match(/ExpandedColumnCount="(\d+)"/);a&&(n=parseInt(a[1]),t?.(`Table has ${n} columns`));let r=e.match(/ExpandedRowCount="(\d+)"/);r&&t?.(`Table has ${r[1]} rows`);let p=/<Row[^>]*>([\s\S]*?)<\/Row>/gi,u;for(;(u=p.exec(e))!==null;){let m=u[1];i=new Array(n).fill("");let f=/<Cell(?:\s+[^>]*ss:Index="(\d+)")?[^>]*>[\s\S]*?<Data[^>]*>([^<]*)<\/Data>[\s\S]*?<\/Cell>|<Cell(?:\s+[^>]*ss:Index="(\d+)")?[^>]*\/>/gi,g,v=0,S=/<Cell(?:[^>]*ss:Index="(\d+)")?[^>]*>(?:[\s\S]*?<Data[^>]*(?:\s+ss:Type="([^"]*)")?[^>]*>([^<]*)<\/Data>)?[\s\S]*?<\/Cell>/gi;for(;(g=S.exec(m))!==null;){g[1]&&(v=parseInt(g[1])-1);let R=g[2]||"",D=g[3]||"";R==="Boolean"&&(D=D==="0"?"False":"True"),v<n&&(i[v]=D),v++}i.some(R=>R.trim())&&s.push([...i]),o++,o%1e4===0&&t?.(`Analyzed ${o.toLocaleString()} rows...`)}return t?.(`XML processing complete: ${s.length} rows, ${n} columns`),this.processedData=s,s}processTextData(e,t){if(t?.("Processing text data..."),!e.trim())return[];let s=e.split(`
`);for(;s.length&&!s[s.length-1].trim();)s.pop();if(!s.length)return[];let n=["	",",",";","|"],i={};for(let p of n){let u=[];for(let m of s.slice(0,Math.min(5,s.length)))if(m.trim()){let f=m.split(p);u.push(f.length)}if(u.length){let m=u.reduce((g,v)=>g+v,0)/u.length,f=u.reduce((g,v)=>g+Math.pow(v-m,2),0)/u.length;i[p]=[m,-f]}}let o="	";Object.keys(i).length&&(o=Object.keys(i).reduce((p,u)=>{let[m,f]=i[p]||[0,0],[g,v]=i[u];return g>m||g===m&&v>f?u:p},"	")),t?.(`Auto-detected delimiter: '${o==="	"?"\\t":o}'`);let a=[],r=0;for(let p of s)if(p.trim()){let u=p.split(o).map(m=>m.trim());a.push(u),r=Math.max(r,u.length)}for(let p of a)for(;p.length<r;)p.push("");return t?.(`Text processing complete: ${a.length} rows, ${r} columns`),this.processedData=a,a}async getClipboardText(){return await ht.env.clipboard.readText()}async processClipboardData(e,t){t?.("Getting clipboard data...");let s=await this.getClipboardText();if(!s)throw new Error("No data found in clipboard");t?.(`Data size: ${s.length} characters`);let n="TEXT";e==="XML Spreadsheet"||!e&&s.includes("<Workbook")&&s.includes("<Worksheet")?n="XML Spreadsheet":e==="TEXT"&&(n="TEXT"),t?.(`Detected format: ${n}`);let i;return n==="XML Spreadsheet"?i=this.processXmlSpreadsheet(s,t):i=this.processTextData(s,t),t?.(`Processed ${i.length} rows`),i.length&&t?.(`Columns per row: ${i[0].length}`),[i,n]}}});var jt={};Q(jt,{activate:()=>Yt,deactivate:()=>Qt});module.exports=Rt(jt);var c=$(require("vscode"));ae();_e();var ie=$(require("vscode")),Ie=class l{constructor(e,t,s){this.extensionUri=t;this.connectionManager=s;this._disposables=[];this._panel=e,this._panel.onDidDispose(()=>this.dispose(),null,this._disposables),this._panel.webview.html=this._getHtmlForWebview(this._panel.webview),this._panel.webview.onDidReceiveMessage(async n=>{switch(n.command){case"save":await this.connectionManager.saveConnection(n.data),ie.window.showInformationMessage("Connection saved!"),this.dispose();return}},null,this._disposables)}static createOrShow(e,t){let s=ie.window.activeTextEditor?ie.window.activeTextEditor.viewColumn:void 0;if(l.currentPanel){l.currentPanel._panel.reveal(s);return}let n=ie.window.createWebviewPanel("netezzaLogin","Connect to Netezza",s||ie.ViewColumn.One,{enableScripts:!0});l.currentPanel=new l(n,e,t)}dispose(){for(l.currentPanel=void 0,this._panel.dispose();this._disposables.length;){let e=this._disposables.pop();e&&e.dispose()}}_getHtmlForWebview(e){return`<!DOCTYPE html>
        <html lang="en">
        <head>
            <meta charset="UTF-8">
...
`&&(a=!1);else if(r)f==="*"&&g==="/"&&(r=!1,m++);else if(i)f==="'"&&e[m-1]!=="\\"&&(i=!1);else if(o)f==='"'&&e[m-1]!=="\\"&&(o=!1);else if(f==="-"&&g==="-")a=!0;else if(f==="/"&&g==="*")r=!0;else if(f==="'")i=!0;else if(f==='"')o=!0;else if(f===";"){n=m;break}}let u=e.substring(s,n).trim();return u?{sql:u,start:s,end:n}:null}static getObjectAtPosition(e,t){let s=p=>/[a-zA-Z0-9_."]/i.test(p),n=t;for(;n>0&&s(e[n-1]);)n--;let i=t;for(;i<e.length&&s(e[i]);)i++;let o=e.substring(n,i);if(!o)return null;let a=p=>p?p.replace(/"/g,""):void 0;if(o.includes("..")){let p=o.split("..");if(p.length===2)return{database:a(p[0]),name:a(p[1])}}let r=o.split(".");return r.length===1?{name:a(r[0])}:r.length===2?{schema:a(r[0]),name:a(r[1])}:r.length===3?{database:a(r[0]),schema:a(r[1]),name:a(r[2])}:null}};var ce=$(require("vscode"));var De=class{provideDocumentLinks(e,t){let s=[],n=e.getText(),i=/[a-zA-Z0-9_"]+(\.[a-zA-Z0-9_"]*)+/g,o;for(;(o=i.exec(n))!==null;){let a=e.positionAt(o.index),r=e.positionAt(o.index+o[0].length),p=new ce.Range(a,r),u=Z.getObjectAtPosition(n,o.index+Math.floor(o[0].length/2));if(u){if(o[0].split(".").length===2&&!u.database&&this.isLikelyAliasReference(n,o.index))continue;let g={name:u.name,schema:u.schema,database:u.database},v=ce.Uri.parse(`command:netezza.revealInSchema?${encodeURIComponent(JSON.stringify(g))}`),S=new ce.DocumentLink(p,v);S.tooltip=`Reveal ${u.name} in Schema`,s.push(S)}}return s}isLikelyAliasReference(e,t){let n=e.substring(Math.max(0,t-200),t).replace(/--[^\n]*/g,"").replace(/\/\*[\s\S]*?\*\//g,"").toUpperCase();return/(?:FROM|JOIN)\s+[a-zA-Z0-9_"]*$/i.test(n)?!1:!!n.match(/\b(SELECT|WHERE|ON|HAVING|ORDER\s+BY|GROUP\s+BY|AND|OR|SET|VALUES)\b(?!.*\b(?:FROM|JOIN)\b)/)}};var Me=$(require("vscode")),xe=class{provideFoldingRanges(e,t,s){let n=[],i=[],o=/^\s*--\s*REGION\b/i,a=/^\s*--\s*ENDREGION\b/i;for(let r=0;r<e.lineCount;r++){let u=e.lineAt(r).text;if(o.test(u))i.push(r);else if(a.test(u)&&i.length>0){let m=i.pop();n.push(new Me.FoldingRange(m,r,Me.FoldingRangeKind.Region))}}return n}};var V=$(require("vscode"));Ae();var fe=class{constructor(e,t){this._extensionUri=e;this._context=t}static{this.viewType="netezza.queryHistory"}resolveWebviewView(e,t,s){this._view=e,e.webview.options={enableScripts:!0,localResourceRoots:[this._extensionUri]},e.webview.html=this._getHtmlForWebview(e.webview),this.sendHistoryToWebview(),e.webview.onDidReceiveMessage(async n=>{switch(n.type){case"refresh":this.refresh();break;case"clearAll":await this.clearAllHistory();break;case"deleteEntry":await this.deleteEntry(n.id,n.query);break;case"copyQuery":await V.env.clipboard.writeText(n.query),V.window.showInformationMessage("Query copied to clipboard");break;case"executeQuery":await this.executeQuery(n.query);break;case"getHistory":await this.sendHistoryToWebview();break;case"toggleFavorite":await this.toggleFavorite(n.id);break;case"updateEntry":await this.updateEntry(n.id,n.tags,n.description);break;case"requestEdit":await this.requestEdit(n.id);break;case"requestTagFilter":await this.requestTagFilter(n.tags);break;case"showFavoritesOnly":await this.sendFavoritesToWebview();break;case"filterByTag":await this.sendFilteredByTagToWebview(n.tag);break}})}refresh(){this._view&&this.sendHistoryToWebview()}async sendHistoryToWebview(){if(!this._view)return;let e=new W(this._context),t=await e.getHistory(),s=await e.getStats();console.log("QueryHistoryView: sending history to webview, entries=",t.length),this._view.webview.postMessage({type:"historyData",history:t,stats:s})}async clearAllHistory(){await V.window.showWarningMessage("Are you sure you want to clear all query history?",{modal:!0},"Clear All")==="Clear All"&&(await new W(this._context).clearHistory(),this.refresh(),V.window.showInformationMessage("Query history cleared"))}async deleteEntry(e,t){let s=t?`: ${t.substring(0,50)}${t.length>50?"...":""}`:"";await V.window.showWarningMessage(`Are you sure you want to delete this query${s}?`,{modal:!0},"Delete")==="Delete"&&(await new W(this._context).deleteEntry(e),this.refresh())}async executeQuery(e){let t=await V.workspace.openTextDocument({content:e,language:"sql"});await V.window.showTextDocument(t)}async toggleFavorite(e){await new W(this._context).toggleFavorite(e),this.refresh()}async updateEntry(e,t,s){await new W(this._context).updateEntry(e,t,s),this.refresh(),V.window.showInformationMessage("Entry updated successfully")}async requestEdit(e){let n=(await new W(this._context).getHistory()).find(a=>a.id===e);if(!n){V.window.showErrorMessage("Entry not found");return}let i=await V.window.showInputBox({prompt:"Enter tags (comma separated)",value:n.tags||"",placeHolder:"tag1, tag2, tag3"});if(i===void 0)return;let o=await V.window.showInputBox({prompt:"Enter description",value:n.description||"",placeHolder:"Description for this query"});o!==void 0&&await this.updateEntry(e,i,o)}async requestTagFilter(e){if(e.length===1)await this.sendFilteredByTagToWebview(e[0]);else if(e.length>1){let t=await V.window.showQuickPick(e,{placeHolder:"Filter by which tag?"});t&&await this.sendFilteredByTagToWebview(t)}}async sendFavoritesToWebview(){if(!this._view)return;let e=new W(this._context),t=await e.getFavorites(),s=await e.getStats();this._view.webview.postMessage({type:"historyData",history:t,stats:s,filter:"favorites"})}async sendFilteredByTagToWebview(e){if(!this._view)return;let t=new W(this._context),s=await t.getByTag(e),n=await t.getStats();this._view.webview.postMessage({type:"historyData",history:s,stats:n,filter:`tag: ${e}`})}_getHtmlForWebview(e){let t=It();return`<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
</html>`}};function It(){let l="",e="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789";for(let t=0;t<32;t++)l+=e.charAt(Math.floor(Math.random()*e.length));return l}var vt=$(require("path"));function wt(l,e){let t=e.getKeepConnectionOpen();l.text=t?"\u{1F517} Keep Connection ON":"\u26D3\uFE0F\u200D\u{1F4A5} Keep Connection OFF",l.tooltip=t?"Keep Connection Open: ENABLED - Click to disable":"Keep Connection Open: DISABLED - Click to enable",l.backgroundColor=t?new c.ThemeColor("statusBarItem.prominentBackground"):void 0}function Yt(l){console.log("Netezza extension: Activating..."),l.subscriptions.push({dispose:()=>{e.closePersistentConnection()}});let e=new oe(l),t=new ge(l,e),s=new me(l.extensionUri),n=c.window.createStatusBarItem(c.StatusBarAlignment.Right,100);n.command="netezza.toggleKeepConnectionOpen",wt(n,e),n.show(),l.subscriptions.push(n),console.log("Netezza extension: Registering SchemaSearchProvider...");let i=new he(l.extensionUri,l);console.log("Netezza extension: Registering QueryHistoryView...");let o=new fe(l.extensionUri,l),a=c.window.createTreeView("netezza.schema",{treeDataProvider:t,showCollapseAll:!0});l.subscriptions.push(c.window.registerWebviewViewProvider(me.viewType,s),c.window.registerWebviewViewProvider(he.viewType,i),c.window.registerWebviewViewProvider(fe.viewType,o));let r=/(^|\s)(?:[A-Za-z]:\\|\\|\/)?[\w.\-\\\/]+\.py\b|(^|\s)python(?:\.exe)?\s+[^\n]*\.py\b/i;function p(d){return d&&(d.includes(" ")?`"${d.replace(/"/g,'\\"')}"`:d)}function u(d,h,w){let y=/[ \\/]/.test(d)?`& ${p(d)}`:d,b=p(h),C=w.map(E=>p(E)).join(" ");return`${y} ${b}${C?" "+C:""}`.trim()}class m{constructor(){this._onDidChange=new c.EventEmitter;this.onDidChangeCodeLenses=this._onDidChange.event}provideCodeLenses(h){let w=[];for(let T=0;T<h.lineCount;T++){let y=h.lineAt(T);if(r.test(y.text)){let b=y.range,C={title:"Run as script",command:"netezza.runScriptFromLens",arguments:[h.uri,b]};w.push(new c.CodeLens(b,C))}}return w}refresh(){this._onDidChange.fire()}}let f=new m;l.subscriptions.push(c.languages.registerCodeLensProvider({scheme:"file"},f));let g=c.window.createTextEditorDecorationType({backgroundColor:new c.ThemeColor("editor.rangeHighlightBackground"),borderRadius:"3px"});function v(d){let h=d||c.window.activeTextEditor;if(!h)return;let w=h.document,T=[];for(let y=0;y<w.lineCount;y++){let b=w.lineAt(y);r.test(b.text)&&T.push({range:b.range,hoverMessage:"Python script invocation"})}h.setDecorations(g,T)}l.subscriptions.push(c.window.onDidChangeActiveTextEditor(()=>v()),c.workspace.onDidChangeTextDocument(d=>{c.window.activeTextEditor&&d.document===c.window.activeTextEditor.document&&v(c.window.activeTextEditor)})),l.subscriptions.push(c.commands.registerCommand("netezza.runScriptFromLens",async(d,h)=>{try{let w=await c.workspace.openTextDocument(d),T=w.getText(h).trim()||w.lineAt(h.start.line).text.trim();if(!T){c.window.showWarningMessage("No script command found");return}let y=T.split(/\s+/),b=y[0]||"",C=/python(\\.exe)?$/i.test(b)&&y.length>=2&&y[1].toLowerCase().endsWith(".py"),E=b.toLowerCase().endsWith(".py"),I=c.workspace.getConfiguration("netezza").get("pythonPath")||"python",M="";if(C){let _=y[0],q=y[1],J=y.slice(2);M=u(_,q,J)}else if(E){let _=b,q=y.slice(1);M=u(I,_,q)}else M=u(I,"",y);let z=c.window.createTerminal({name:"Netezza: Script"});z.show(!0),z.sendText(M,!0),c.window.showInformationMessage(`Running script: ${M}`)}catch(w){c.window.showErrorMessage(`Error running script: ${w.message}`)}})),v(c.window.activeTextEditor),l.subscriptions.push(c.window.onDidChangeActiveTextEditor(d=>{if(d&&d.document){let h=d.document.uri.toString();s.setActiveSource(h)}})),l.subscriptions.push(c.commands.registerCommand("netezza.toggleKeepConnectionOpen",()=>{let d=e.getKeepConnectionOpen();e.setKeepConnectionOpen(!d),wt(n,e);let h=!d;c.window.showInformationMessage(h?"Keep connection open: ENABLED - connection will remain open after queries":"Keep connection open: DISABLED - connection will be closed after each query")}),c.commands.registerCommand("netezza.openLogin",()=>{Ie.createOrShow(l.extensionUri,e)}),c.commands.registerCommand("netezza.refreshSchema",()=>{t.refresh(),c.window.showInformationMessage("Schema refreshed")}),c.commands.registerCommand("netezza.copySelectAll",d=>{if(d&&d.label&&d.dbName&&d.schema){let h=`SELECT * FROM ${d.dbName}.${d.schema}.${d.label} LIMIT 100;`;c.env.clipboard.writeText(h),c.window.showInformationMessage("Copied to clipboard")}}),c.commands.registerCommand("netezza.copyDrop",async d=>{if(d&&d.label&&d.dbName&&d.schema&&d.objType){let h=`${d.dbName}.${d.schema}.${d.label}`,w=`DROP ${d.objType} ${h};`;if(await c.window.showWarningMessage(`Are you sure you want to delete ${d.objType.toLowerCase()} "${h}"?`,{modal:!0},"Yes, delete","Cancel")==="Yes, delete")try{if(!await e.getConnectionString()){c.window.showErrorMessage("No database connection");return}await c.window.withProgress({location:c.ProgressLocation.Notification,title:`Deleting ${d.objType.toLowerCase()} ${h}...`,cancellable:!1},async b=>{await L(l,w,!0)}),c.window.showInformationMessage(`Deleted ${d.objType.toLowerCase()}: ${h}`),t.refresh()}catch(y){c.window.showErrorMessage(`Error during deletion: ${y.message}`)}}}),c.commands.registerCommand("netezza.copyName",d=>{if(d&&d.label&&d.dbName&&d.schema){let h=`${d.dbName}.${d.schema}.${d.label}`;c.env.clipboard.writeText(h),c.window.showInformationMessage("Copied to clipboard")}}),c.commands.registerCommand("netezza.grantPermissions",async d=>{if(d&&d.label&&d.dbName&&d.schema&&d.objType){let h=`${d.dbName}.${d.schema}.${d.label}`,w=await c.window.showQuickPick([{label:"SELECT",description:"Privileges to read data"},{label:"INSERT",description:"Privileges to insert data"},{label:"UPDATE",description:"Privileges to update data"},{label:"DELETE",description:"Privileges to delete data"},{label:"ALL",description:"All privileges (SELECT, INSERT, UPDATE, DELETE)"},{label:"LIST",description:"Privileges to list objects"}],{placeHolder:"Select privilege type"});if(!w)return;let T=await c.window.showInputBox({prompt:"Enter user or group name",placeHolder:"e.g. SOME_USER or GROUP_NAME",validateInput:C=>!C||C.trim().length===0?"User/group name cannot be empty":/^[a-zA-Z_][a-zA-Z0-9_]*$/.test(C.trim())?null:"Invalid user/group name"});if(!T)return;let y=`GRANT ${w.label} ON ${h} TO ${T.trim().toUpperCase()};`;if(await c.window.showInformationMessage(`Execute: ${y}`,{modal:!0},"Yes, execute","Cancel")==="Yes, execute")try{if(!await e.getConnectionString()){c.window.showErrorMessage("No database connection");return}await c.window.withProgress({location:c.ProgressLocation.Notification,title:`Granting ${w.label} on ${h}...`,cancellable:!1},async E=>{await L(l,y,!0)}),c.window.showInformationMessage(`Granted ${w.label} on ${h} to ${T.trim().toUpperCase()}`)}catch(C){c.window.showErrorMessage(`Error granting privileges: ${C.message}`)}}}),c.commands.registerCommand("netezza.groomTable",async d=>{if(d&&d.label&&d.dbName&&d.schema&&d.objType==="TABLE"){let h=`${d.dbName}.${d.schema}.${d.label}`,w=await c.window.showQuickPick([{label:"RECORDS ALL",description:"Groom all records (reclaim space from deleted rows)"},{label:"RECORDS READY",description:"Groom only ready records"},{label:"PAGES ALL",description:"Groom all pages (reorganize data pages)"},{label:"PAGES START",description:"Groom pages from start"},{label:"VERSIONS",description:"Groom versions (clean up old row versions)"}],{placeHolder:"Select GROOM mode"});if(!w)return;let T=await c.window.showQuickPick([{label:"DEFAULT",description:"Use default backupset",value:"DEFAULT"},{label:"NONE",description:"No backupset",value:"NONE"},{label:"Custom",description:"Specify custom backupset ID",value:"CUSTOM"}],{placeHolder:"Select RECLAIM BACKUPSET option"});if(!T)return;let y=T.value;if(T.value==="CUSTOM"){let E=await c.window.showInputBox({prompt:"Enter backupset ID",placeHolder:"np. 12345",validateInput:A=>!A||A.trim().length===0?"Backupset ID cannot be empty":/^\d+$/.test(A.trim())?null:"Backupset ID must be a number"});if(!E)return;y=E.trim()}let b=`GROOM TABLE ${h} ${w.label} RECLAIM BACKUPSET ${y};`;if(await c.window.showWarningMessage(`Execute GROOM on table "${h}"?

${b}

Warning: This operation may be time-consuming for large tables.`,{modal:!0},"Yes, execute","Cancel")==="Yes, execute")try{if(!await e.getConnectionString()){c.window.showErrorMessage("No database connection");return}let A=Date.now();await c.window.withProgress({location:c.ProgressLocation.Notification,title:`GROOM TABLE ${h} (${w.label})...`,cancellable:!1},async M=>{await L(l,b,!0)});let I=((Date.now()-A)/1e3).toFixed(1);c.window.showInformationMessage(`GROOM completed successfully (${I}s): ${h}`)}catch(E){c.window.showErrorMessage(`Error during GROOM: ${E.message}`)}}}),c.commands.registerCommand("netezza.addTableComment",async d=>{if(d&&d.label&&d.dbName&&d.schema&&d.objType==="TABLE"){let h=`${d.dbName}.${d.schema}.${d.label}`,w=await c.window.showInputBox({prompt:"Enter comment for table",placeHolder:"e.g. Table contains customer data",value:d.objectDescription||""});if(w===void 0)return;let T=`COMMENT ON TABLE ${h} IS '${w.replace(/'/g,"''")}';`;try{if(!await e.getConnectionString()){c.window.showErrorMessage("No database connection");return}await L(l,T,!0),c.window.showInformationMessage(`Comment added to table: ${h}`),t.refresh()}catch(y){c.window.showErrorMessage(`Error adding comment: ${y.message}`)}}}),c.commands.registerCommand("netezza.generateStatistics",async d=>{if(d&&d.label&&d.dbName&&d.schema&&d.objType==="TABLE"){let h=`${d.dbName}.${d.schema}.${d.label}`,w=`GENERATE EXPRESS STATISTICS ON ${h};`;if(await c.window.showInformationMessage(`Generate statistics for table "${h}"?

${w}`,{modal:!0},"Yes, generate","Cancel")==="Yes, generate")try{if(!await e.getConnectionString()){c.window.showErrorMessage("No database connection");return}let b=Date.now();await c.window.withProgress({location:c.ProgressLocation.Notification,title:`Generating statistics for ${h}...`,cancellable:!1},async E=>{await L(l,w,!0)});let C=((Date.now()-b)/1e3).toFixed(1);c.window.showInformationMessage(`Statistics generated successfully (${C}s): ${h}`)}catch(y){c.window.showErrorMessage(`Error generating statistics: ${y.message}`)}}}),c.commands.registerCommand("netezza.truncateTable",async d=>{if(d&&d.label&&d.dbName&&d.schema&&d.objType==="TABLE"){let h=`${d.dbName}.${d.schema}.${d.label}`,w=`TRUNCATE TABLE ${h};`;if(await c.window.showWarningMessage(`\u26A0\uFE0F WARNING: Are you sure you want to delete ALL data from the table "${h}"?

${w}

This operation is IRREVERSIBLE!`,{modal:!0},"Yes, delete all data","Cancel")==="Yes, delete all data")try{if(!await e.getConnectionString()){c.window.showErrorMessage("No database connection");return}await c.window.withProgress({location:c.ProgressLocation.Notification,title:`Clearing table ${h}...`,cancellable:!1},async b=>{await L(l,w,!0)}),c.window.showInformationMessage(`Table cleared: ${h}`)}catch(y){c.window.showErrorMessage(`Error clearing table: ${y.message}`)}}}),c.commands.registerCommand("netezza.addPrimaryKey",async d=>{if(d&&d.label&&d.dbName&&d.schema&&d.objType==="TABLE"){let h=`${d.dbName}.${d.schema}.${d.label}`,w=await c.window.showInputBox({prompt:"Enter primary key constraint name",placeHolder:`e.g. PK_${d.label}`,value:`PK_${d.label}`,validateInput:E=>!E||E.trim().length===0?"Constraint name cannot be empty":/^[a-zA-Z_][a-zA-Z0-9_]*$/.test(E.trim())?null:"Invalid constraint name"});if(!w)return;let T=await c.window.showInputBox({prompt:"Enter primary key column names (comma separated)",placeHolder:"e.g. COL1, COL2 or ID",validateInput:E=>!E||E.trim().length===0?"You must provide at least one column":null});if(!T)return;let y=T.split(",").map(E=>E.trim().toUpperCase()).join(", "),b=`ALTER TABLE ${h} ADD CONSTRAINT ${w.trim().toUpperCase()} PRIMARY KEY (${y});`;if(await c.window.showInformationMessage(`Add primary key to table "${h}"?

${b}`,{modal:!0},"Yes, add","Cancel")==="Yes, add")try{if(!await e.getConnectionString()){c.window.showErrorMessage("No database connection");return}await c.window.withProgress({location:c.ProgressLocation.Notification,title:`Adding primary key to ${h}...`,cancellable:!1},async A=>{await L(l,b,!0)}),c.window.showInformationMessage(`Primary key added: ${w.trim().toUpperCase()}`),t.refresh()}catch(E){c.window.showErrorMessage(`Error adding primary key: ${E.message}`)}}}),c.commands.registerCommand("netezza.createDDL",async d=>{try{if(!d||!d.label||!d.dbName||!d.schema||!d.objType){c.window.showErrorMessage("Invalid object selected for DDL generation");return}let h=await e.getConnectionString();if(!h){c.window.showErrorMessage("Connection not configured. Please connect via Netezza: Connect...");return}await c.window.withProgress({location:c.ProgressLocation.Notification,title:`Generating DDL for ${d.objType} ${d.label}...`,cancellable:!1},async()=>{let{generateDDL:w}=await Promise.resolve().then(()=>(lt(),ct)),T=await w(h,d.dbName,d.schema,d.label,d.objType);if(T.success&&T.ddlCode){let y=await c.window.showQuickPick([{label:"Open in Editor",description:"Open DDL code in a new editor",value:"editor"},{label:"Copy to Clipboard",description:"Copy DDL code to clipboard",value:"clipboard"}],{placeHolder:"How would you like to access the DDL code?"});if(y)if(y.value==="editor"){let b=await c.workspace.openTextDocument({content:T.ddlCode,language:"sql"});await c.window.showTextDocument(b),c.window.showInformationMessage(`DDL code generated for ${d.objType} ${d.label}`)}else y.value==="clipboard"&&(await c.env.clipboard.writeText(T.ddlCode),c.window.showInformationMessage("DDL code copied to clipboard"))}else throw new Error(T.error||"DDL generation failed")})}catch(h){c.window.showErrorMessage(`Error generating DDL: ${h.message}`)}}),c.commands.registerCommand("netezza.revealInSchema",async d=>{try{if(!await e.getConnectionString()){c.window.showWarningMessage("Not connected to database");return}let w=d.name,T=d.objType,y=T?[T]:["TABLE","VIEW","EXTERNAL TABLE","PROCEDURE","FUNCTION","SEQUENCE","SYNONYM"];if(T==="COLUMN"){if(!d.parent){c.window.showWarningMessage("Cannot find column without parent table");return}w=d.parent}let b=[];if(d.database)b=[d.database];else{let C=await L(l,"SELECT DATABASE FROM system.._v_database ORDER BY DATABASE",!0);if(!C)return;b=JSON.parse(C).map(A=>A.DATABASE)}for(let C of b)try{let E=T==="COLUMN"?["TABLE","VIEW","EXTERNAL TABLE"]:y;for(let A of E){let I=`SELECT OBJNAME, OBJTYPE, SCHEMA, OBJID FROM ${C}.._V_OBJECT_DATA WHERE UPPER(OBJNAME) = UPPER('${w.replace(/'/g,"''")}') AND UPPER(OBJTYPE) = UPPER('${A}') AND DBNAME = '${C}'`;d.schema&&(I+=` AND UPPER(SCHEMA) = UPPER('${d.schema.replace(/'/g,"''")}')`);let M=await L(l,I,!0);if(M){let z=JSON.parse(M);if(z.length>0){let _=z[0],{SchemaItem:q}=await Promise.resolve().then(()=>(Ue(),ot)),J=new q(_.OBJNAME,c.TreeItemCollapsibleState.Collapsed,`netezza:${_.OBJTYPE}`,C,_.OBJTYPE,_.SCHEMA,_.OBJID);await a.reveal(J,{select:!0,focus:!0,expand:!0});return}}}}catch(E){console.log(`Error searching in ${C}:`,E)}c.window.showWarningMessage(`Could not find ${T||"object"} ${w}`)}catch(h){c.window.showErrorMessage(`Error revealing item: ${h.message}`)}}),c.commands.registerCommand("netezza.showQueryHistory",()=>{c.commands.executeCommand("netezza.queryHistory.focus")}),c.commands.registerCommand("netezza.clearQueryHistory",async()=>{let{QueryHistoryManager:d}=await Promise.resolve().then(()=>(Ae(),je)),h=new d(l);await c.window.showWarningMessage("Are you sure you want to clear all query history?",{modal:!0},"Clear All")==="Clear All"&&(await h.clearHistory(),o.refresh(),c.window.showInformationMessage("Query history cleared"))})),l.subscriptions.push(c.languages.registerDocumentLinkProvider({language:"sql"},new De)),l.subscriptions.push(c.languages.registerFoldingRangeProvider({language:"sql"},new xe)),l.subscriptions.push(c.commands.registerCommand("netezza.jumpToSchema",async()=>{let d=c.window.activeTextEditor;if(!d)return;let h=d.document,w=d.selection,T=h.offsetAt(w.active),y=Z.getObjectAtPosition(h.getText(),T);y?c.commands.executeCommand("netezza.revealInSchema",y):c.window.showWarningMessage("No object found at cursor")}));let S=c.commands.registerCommand("netezza.runQuery",async()=>{let d=c.window.activeTextEditor;if(!d){c.window.showErrorMessage("No active editor found");return}let h=d.document,w=d.selection,T=h.getText(),y=h.uri.toString(),b=[];if(w.isEmpty){let E=h.offsetAt(w.active),A=Z.getStatementAtPosition(T,E);if(A){b=[A.sql];let I=h.positionAt(A.start),M=h.positionAt(A.end);d.selection=new c.Selection(I,M)}else{c.window.showWarningMessage("No SQL statement found at cursor");return}}else{let E=h.getText(w);if(!E.trim()){c.window.showWarningMessage("No SQL query selected");return}b=Z.splitStatements(E)}if(b.length===0)return;let C=b.length===1?b[0].trim():null;if(C){let E=C.split(/\s+/),A=E[0]||"",I=/python(\.exe)?$/i.test(A)&&E.length>=2&&E[1].toLowerCase().endsWith(".py"),M=A.toLowerCase().endsWith(".py");if(I||M){let _=c.workspace.getConfiguration("netezza").get("pythonPath")||"python",q="";if(I){let se=E[0],ue=E[1],yt=E.slice(2);q=u(se,ue,yt)}else{let se=A,ue=E.slice(1);q=u(_,se,ue)}let J=c.window.createTerminal({name:"Netezza: Script"});J.show(!0),J.sendText(q,!0),c.window.showInformationMessage(`Running script: ${q}`);return}}try{let E=await ze(l,b,e);s.updateResults(E,y,!1),c.commands.executeCommand("netezza.results.focus")}catch(E){c.window.showErrorMessage(`Error executing query: ${E.message}`)}}),R=c.commands.registerCommand("netezza.runQueryBatch",async()=>{let d=c.window.activeTextEditor;if(!d){c.window.showErrorMessage("No active editor found");return}let h=d.document,w=d.selection,T=h.uri.toString(),y;if(w.isEmpty?y=h.getText():y=h.getText(w),!y.trim()){c.window.showWarningMessage("No SQL query to execute");return}let C=y.trim().split(/\s+/),E=C[0]||"",A=/python(\.exe)?$/i.test(E)&&C.length>=2&&C[1].toLowerCase().endsWith(".py"),I=E.toLowerCase().endsWith(".py");if(A||I){let z=c.workspace.getConfiguration("netezza").get("pythonPath")||"python",_="";if(A){let J=C[0],se=C[1],ue=C.slice(2);_=u(J,se,ue)}else{let J=E,se=C.slice(1);_=u(z,J,se)}let q=c.window.createTerminal({name:"Netezza: Script"});q.show(!0),q.sendText(_,!0),c.window.showInformationMessage(`Running script: ${_}`);return}try{let{runQueryRaw:M}=await Promise.resolve().then(()=>(ae(),st)),z=await M(l,y,!1,e);z&&(s.updateResults([z],T,!1),c.commands.executeCommand("netezza.results.focus"))}catch(M){c.window.showErrorMessage(`Error executing query: ${M.message}`)}});l.subscriptions.push(R);let D=c.window.createOutputChannel("Netezza"),N=(d,h)=>{let w=Date.now()-h;D.appendLine(`[${new Date().toLocaleTimeString()}] ${d} completed in ${w}ms`),D.show(!0)},x=c.commands.registerCommand("netezza.exportToXlsb",async()=>{let d=c.window.activeTextEditor;if(!d){c.window.showErrorMessage("No active editor found");return}let h=d.selection,w=h.isEmpty?d.document.getText():d.document.getText(h);if(!w.trim()){c.window.showWarningMessage("No SQL query to export");return}let T=await c.window.showSaveDialog({filters:{"Excel Workbook":["xlsx"]},saveLabel:"Export to XLSX"});if(!T)return;let y=Date.now();try{let b=await e.getConnectionString();if(!b)throw new Error("Connection not configured. Please connect via Netezza: Connect...");await c.window.withProgress({location:c.ProgressLocation.Notification,title:"Exporting to XLSX...",cancellable:!1},async C=>{let{exportQueryToXlsb:E}=await Promise.resolve().then(()=>(we(),Ee)),A=await E(b,w,T.fsPath,!1,I=>{C.report({message:I}),D.appendLine(`[XLSX Export] ${I}`)});if(!A.success)throw new Error(A.message)}),N("Export to XLSX",y),c.window.showInformationMessage(`Results exported to ${T.fsPath}`)}catch(b){c.window.showErrorMessage(`Error exporting to XLSX: ${b.message}`)}}),H=c.commands.registerCommand("netezza.exportToCsv",async()=>{let d=c.window.activeTextEditor;if(!d){c.window.showErrorMessage("No active editor found");return}let h=d.selection,w=h.isEmpty?d.document.getText():d.document.getText(h);if(!w.trim()){c.window.showWarningMessage("No SQL query to export");return}let T=await c.window.showSaveDialog({filters:{"CSV Files":["csv"]},saveLabel:"Export to CSV"});if(!T)return;let y=Date.now();try{let b=await e.getConnectionString();if(!b)throw new Error("Connection not configured. Please connect via Netezza: Connect...");await c.window.withProgress({location:c.ProgressLocation.Notification,title:"Exporting to CSV...",cancellable:!1},async C=>{let{exportToCsv:E}=await Promise.resolve().then(()=>(gt(),pt));await E(l,b,w,T.fsPath,C)}),N("Export to CSV",y),c.window.showInformationMessage(`Results exported to ${T.fsPath}`)}catch(b){c.window.showErrorMessage(`Error exporting to CSV: ${b.message}`)}}),te=c.commands.registerCommand("netezza.copyXlsbToClipboard",async()=>{let d=c.window.activeTextEditor;if(!d){c.window.showErrorMessage("No active editor found");return}let h=d.selection,w=h.isEmpty?d.document.getText():d.document.getText(h);if(!w.trim()){c.window.showWarningMessage("No SQL query to export");return}try{let T=await e.getConnectionString();if(!T)throw new Error("Connection not configured. Please connect via Netezza: Connect...");let y=Date.now();if(await c.window.withProgress({location:c.ProgressLocation.Notification,title:"Exporting to XLSX and copying to clipboard...",cancellable:!1},async C=>{let{exportQueryToXlsb:E,getTempFilePath:A}=await Promise.resolve().then(()=>(we(),Ee)),I=A(),M=await E(T,w,I,!0,z=>{C.report({message:z}),D.appendLine(`[XLSX Clipboard] ${z}`)});if(!M.success)throw new Error(M.message);if(!M.details?.clipboard_success)throw new Error("Failed to copy file to clipboard")}),N("Copy XLSX to Clipboard",y),await c.window.showInformationMessage("Excel file copied to clipboard! You can now paste it into Excel or Windows Explorer.","Show Temp Folder","OK")==="Show Temp Folder"){let C=require("os").tmpdir();await c.env.openExternal(c.Uri.file(C))}}catch(T){c.window.showErrorMessage(`Error copying XLSX to clipboard: ${T.message}`)}}),ye=c.commands.registerCommand("netezza.exportToXlsbAndOpen",async()=>{let d=c.window.activeTextEditor;if(!d){c.window.showErrorMessage("No active editor found");return}let h=d.selection,w=h.isEmpty?d.document.getText():d.document.getText(h);if(!w.trim()){c.window.showWarningMessage("No SQL query to export");return}let T=await c.window.showSaveDialog({filters:{"Excel Workbook":["xlsx"]},saveLabel:"Export to XLSX and Open"});if(!T)return;let y=Date.now();try{let b=await e.getConnectionString();if(!b)throw new Error("Connection not configured. Please connect via Netezza: Connect...");await c.window.withProgress({location:c.ProgressLocation.Notification,title:"Exporting to XLSX and opening...",cancellable:!1},async C=>{let{exportQueryToXlsb:E}=await Promise.resolve().then(()=>(we(),Ee)),A=await E(b,w,T.fsPath,!1,I=>{C.report({message:I}),D.appendLine(`[XLSX Export] ${I}`)});if(!A.success)throw new Error(A.message)}),N("Export to XLSX and Open",y),await c.env.openExternal(T),c.window.showInformationMessage(`Results exported and opened: ${T.fsPath}`)}catch(b){c.window.showErrorMessage(`Error exporting to XLSX: ${b.message}`)}}),Pe=c.commands.registerCommand("netezza.importClipboard",async()=>{try{let d=await e.getConnectionString();if(!d)throw new Error("Connection not configured. Please connect via Netezza: Connect...");let h=await c.window.showInputBox({prompt:"Enter target table name (leave empty for auto-generated name)",placeHolder:"e.g. my_schema.my_table or leave empty",validateInput:b=>!b||b.trim().length===0||/^[a-zA-Z_][a-zA-Z0-9_]*(\.[a-zA-Z_][a-zA-Z0-9_]*)?(\.[a-zA-Z_][a-zA-Z0-9_]*)?$/.test(b.trim())?null:"Invalid table name format. Use: [database.]schema.table"});if(h===void 0)return;let w;if(!h||h.trim().length===0)try{let C=await L(l,"SELECT CURRENT_CATALOG, CURRENT_SCHEMA",!0);if(C){let E=JSON.parse(C);if(E&&E.length>0){let A=E[0].CURRENT_CATALOG||"SYSTEM",I=E[0].CURRENT_SCHEMA||"ADMIN",z=new Date().toISOString().slice(0,10).replace(/-/g,""),_=Math.floor(Math.random()*1e4).toString().padStart(4,"0");w=`${A}.${I}.IMPORT_${z}_${_}`,c.window.showInformationMessage(`Auto-generated table name: ${w}`)}else throw new Error("Could not determine current database/schema")}else throw new Error("Could not determine current database/schema")}catch(b){c.window.showErrorMessage(`Error getting current database/schema: ${b.message}`);return}else w=h.trim();let T=await c.window.showQuickPick([{label:"Auto-detect",description:"Automatically detect clipboard format (text or Excel XML)",value:null},{label:"Excel XML Spreadsheet",description:"Force Excel XML format processing",value:"XML Spreadsheet"},{label:"Plain Text",description:"Force plain text processing with delimiter detection",value:"TEXT"}],{placeHolder:"Select clipboard data format"});if(!T)return;let y=Date.now();await c.window.withProgress({location:c.ProgressLocation.Notification,title:"Importing clipboard data...",cancellable:!1},async b=>{let{importClipboardDataToNetezza:C}=await Promise.resolve().then(()=>(Et(),ft)),E=await C(w,d,T.value,{},A=>{b.report({message:A}),D.appendLine(`[Clipboard Import] ${A}`)});if(!E.success)throw new Error(E.message);E.details&&(D.appendLine(`[Clipboard Import] Rows processed: ${E.details.rowsProcessed}`),D.appendLine(`[Clipboard Import] Columns: ${E.details.columns}`),D.appendLine(`[Clipboard Import] Format: ${E.details.format}`))}),N("Import Clipboard Data",y),c.window.showInformationMessage(`Clipboard data imported successfully to table: ${w}`)}catch(d){c.window.showErrorMessage(`Error importing clipboard data: ${d.message}`)}}),Te=c.commands.registerCommand("netezza.importData",async()=>{try{let d=await e.getConnectionString();if(!d)throw new Error("Connection not configured. Please connect via Netezza: Connect...");let h=await c.window.showOpenDialog({canSelectFiles:!0,canSelectFolders:!1,canSelectMany:!1,filters:{"Data Files":["csv","txt","xlsx","xlsb","json"],"CSV Files":["csv"],"Excel Files":["xlsx","xlsb"],"Text Files":["txt"],"JSON Files":["json"],"All Files":["*"]},openLabel:"Select file to import"});if(!h||h.length===0)return;let w=h[0].fsPath,T=await c.window.showInputBox({prompt:"Enter target table name (leave empty for auto-generated name)",placeHolder:"e.g. my_schema.my_table or leave empty",validateInput:E=>!E||E.trim().length===0||/^[a-zA-Z_][a-zA-Z0-9_]*(\.[a-zA-Z_][a-zA-Z0-9_]*)?(\.[a-zA-Z_][a-zA-Z0-9_]*)?$/.test(E.trim())?null:"Invalid table name format. Use: [database.]schema.table"});if(T===void 0)return;let y;if(!T||T.trim().length===0)try{let A=await L(l,"SELECT CURRENT_CATALOG, CURRENT_SCHEMA",!0);if(A){let I=JSON.parse(A);if(I&&I.length>0){let M=I[0].CURRENT_CATALOG||"SYSTEM",z=I[0].CURRENT_SCHEMA||"ADMIN",q=new Date().toISOString().slice(0,10).replace(/-/g,""),J=Math.floor(Math.random()*1e4).toString().padStart(4,"0");y=`${M}.${z}.IMPORT_${q}_${J}`,c.window.showInformationMessage(`Auto-generated table name: ${y}`)}else throw new Error("Could not determine current database/schema")}else throw new Error("Could not determine current database/schema")}catch(E){c.window.showErrorMessage(`Error getting current database/schema: ${E.message}`);return}else y=T.trim();let b=await c.window.showQuickPick([{label:"Default Import",description:"Use default settings",value:{}},{label:"Custom Options",description:"Configure import settings (coming soon)",value:null}],{placeHolder:"Select import options"});if(!b)return;if(b.value===null){c.window.showInformationMessage("Custom options will be available in future version");return}let C=Date.now();await c.window.withProgress({location:c.ProgressLocation.Notification,title:"Importing data...",cancellable:!1},async E=>{let{importDataToNetezza:A}=await Promise.resolve().then(()=>(We(),mt)),I=await A(w,y,d,b.value||{},M=>{E.report({message:M}),D.appendLine(`[Import] ${M}`)});if(!I.success)throw new Error(I.message);I.details&&(D.appendLine(`[Import] Rows processed: ${I.details.rowsProcessed}`),D.appendLine(`[Import] Columns: ${I.details.columns}`),D.appendLine(`[Import] Delimiter: ${I.details.detectedDelimiter}`))}),N("Import Data",C),c.window.showInformationMessage(`Data imported successfully to table: ${y}`)}catch(d){c.window.showErrorMessage(`Error importing data: ${d.message}`)}}),be=c.commands.registerCommand("netezza.exportCurrentResultToXlsbAndOpen",async d=>{try{if(!d){c.window.showErrorMessage("No data to export");return}let h=require("os"),w=require("path"),T=new Date().toISOString().replace(/[:.]/g,"-"),y=w.join(h.tmpdir(),`netezza_results_${T}.xlsx`),b=Date.now();await c.window.withProgress({location:c.ProgressLocation.Notification,title:"Creating Excel file...",cancellable:!1},async E=>{let{exportCsvToXlsb:A}=await Promise.resolve().then(()=>(we(),Ee)),I=await A(d,y,!1,{source:"Query Results Panel"},M=>{E.report({message:M}),D.appendLine(`[CSV to XLSX] ${M}`)});if(!I.success)throw new Error(I.message)});let C=Date.now()-b;D.appendLine(`[${new Date().toLocaleTimeString()}] Export Current Result to Excel completed in ${C}ms`),await c.env.openExternal(c.Uri.file(y)),c.window.showInformationMessage(`Results exported and opened: ${y}`)}catch(h){c.window.showErrorMessage(`Error exporting to Excel: ${h.message}`)}});l.subscriptions.push(S),l.subscriptions.push(x),l.subscriptions.push(H),l.subscriptions.push(te),l.subscriptions.push(ye),l.subscriptions.push(be),l.subscriptions.push(Pe),l.subscriptions.push(Te);let Ye=c.workspace.onWillSaveTextDocument(async d=>{}),B=c.commands.registerCommand("netezza.smartPaste",async()=>{try{let d=c.window.activeTextEditor;if(!d)return;let w=c.workspace.getConfiguration("netezza").get("pythonPath")||"python",T=vt.join(l.extensionPath,"python","check_clipboard_format.py"),y=require("child_process");if(await new Promise(C=>{let E=y.spawn(w,[T]);E.on("close",A=>{C(A===1)}),E.on("error",()=>{C(!1)})})){let C=await c.window.showQuickPick([{label:"\u{1F4CA} import to Netezza table",description:'Detected "XML Spreadsheet" format - import data to database',value:"import"},{label:"\u{1F4DD} Paste as text",description:"Paste clipboard content as plain text",value:"paste"}],{placeHolder:'Detected "XML Spreadsheet" format in clipboard - choose an action'});if(C?.value==="import")c.commands.executeCommand("netezza.importClipboard");else if(C?.value==="paste"){let E=await c.env.clipboard.readText(),A=d.selection;await d.edit(I=>{I.replace(A,E)})}}else{let C=await c.env.clipboard.readText(),E=d.selection;await d.edit(A=>{A.replace(E,C)})}}catch(d){c.window.showErrorMessage(`Error during paste: ${d.message}`)}}),ne=c.workspace.onDidChangeTextDocument(async d=>{if(d.document.languageId!=="sql"&&d.document.languageId!=="mssql"||d.contentChanges.length!==1)return;let h=d.contentChanges[0];if(h.text!==" ")return;let w=c.window.activeTextEditor;if(!w||w.document!==d.document)return;let y=d.document.lineAt(h.range.start.line).text,b=new Map([["SX","SELECT"],["WX","WHERE"],["GX","GROUP BY"],["HX","HAVING"],["OX","ORDER BY"],["FX","FROM"],["JX","JOIN"],["LX","LIMIT"],["IX","INSERT INTO"],["UX","UPDATE"],["DX","DELETE FROM"],["CX","CREATE TABLE"]]);for(let[C,E]of b)if(new RegExp(`\\b${C}\\s$`,"i").test(y)){let I=y.toUpperCase().lastIndexOf(C.toUpperCase());if(I>=0){let M=new c.Position(h.range.start.line,I),z=new c.Position(h.range.start.line,I+C.length+1);await w.edit(_=>{_.replace(new c.Range(M,z),E+" ")}),["SELECT","FROM","JOIN"].includes(E)&&setTimeout(()=>{c.commands.executeCommand("editor.action.triggerSuggest")},100);break}}});l.subscriptions.push(B),l.subscriptions.push(ne),l.subscriptions.push(c.languages.registerCompletionItemProvider(["sql","mssql"],new Ne(l),"."," "))}function Qt(){}0&&(module.exports={activate,deactivate});
