...
class Handshake {
    ...
    private async _readExactly(length: number): Promise<Buffer> {
        return new Promise((resolve, reject) => {
            const buf = this._stream.read(length) as Buffer | null;
            if (buf) {
                resolve(buf);
            } else {
                this._stream.once('readable', () => {
                    const buf2 = this._stream.read(length) as Buffer | null;
    ...
    connSetNextDataProtocol(p1: number, p2: number): boolean {
