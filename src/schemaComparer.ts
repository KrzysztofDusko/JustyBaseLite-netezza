...
import { getColumns, getKeysInfo, getDistributionInfo, getOrganizeInfo, quoteNameIfNeeded } from './ddlGenerator';
 ...
 */
async function getTableMetadata(connection: any, database: string, schema: string, tableName: string): Promise<TableMetadata> {
 ...
 */
async function getProcedureMetadata(connection: any, database: string, schema: string, procSignature: string): Promise<ProcedureMetadata> {
    const sql = `
        SELECT 
            SCHEMA,
            PROCEDURESOURCE,
