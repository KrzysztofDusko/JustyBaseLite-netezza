...
export function parseExplainOutput(text: string): ParsedPlan {
    ...
    const planLines = planTextStart >= 0
    ...
    for (const line of planLines) {
        ...
        const nodeMatch = line.match(/^[\s]*(?:[lr]:\s*)?([\w\s-]+?)(?:\s+table\s+"(\w+)")?(?:\s+{[^}]*})?\s*\(cost=([\d.]+)\.\.([\d.]+)\s+rows=(\d+)\s+width=(\d+)\s+conf=([\d.]+)\)/i);
...
function renderPlanTree(nodes: ExplainNode[], depth: number = 0): string {
    return nodes.map(node => {
        ...
        const warningBadges = node.warnings.map(w =>
            `<span class="warning-badge warning-${w.type}" title="${w.message}">${w.icon}</span>`
        ).join('');
