...
export class SessionMonitorView {
    ...
    private async _fetchResources(): Promise<any> {
        ...
        try {
            const graResult = await runQuery(this._context,
                `SELECT * FROM _V_SCHED_GRA_EXT LIMIT 50`,
                true, undefined, this._connectionManager);
        ...
        try {
            const sysResult = await runQuery(this._context,
        ...
        try {
            const summaryResult = await runQuery(this._context,
                `SELECT 
    ...
    private _getHtmlForWebview(webview: vscode.Webview): string {
        const scriptUri = webview.asWebviewUri(
            vscode.Uri.joinPath(this._extensionUri, 'media', 'sessionMonitor.js')
        );
        const styleUri = webview.asWebviewUri(
            vscode.Uri.joinPath(this._extensionUri, 'media', 'sessionMonitor.css')
        );
