...
export class EditDataProvider {
                ...
                switch (message.command) {
                    ...
                    case 'updateTableComment':
                        await this._execSimpleCommand(
                            context, connectionManager, item.connectionName,
                    ...
                    case 'addColumn':
                        ...
                        if (true) this._loadData(panel, database, schema, tableName, item.connectionName, context, connectionManager);
                        break;
                    case 'dropColumn':
                        await this._execSimpleCommand(
                            context, connectionManager, item.connectionName,
    ...
    private static async _execSimpleCommand(context: vscode.ExtensionContext, connectionManager: ConnectionManager, connectionName: string, sql: string, successMsg: string, refresh = false) {
        try {
    ...
    private static async _loadData(panel: vscode.WebviewPanel, db: string, schema: string, table: string, connectionName: string, context: vscode.ExtensionContext, connectionManager: ConnectionManager) {
        const fullTableName = `${db}.${schema}.${table}`;
        try {
            panel.webview.postMessage({ command: 'setLoading', loading: true, message: 'Fetching data...' });

            const metadataQuery = `
                SELECT description FROM ${db}.._v_object_data WHERE objtype='TABLE' AND objname='${table}' AND schema='${schema}';
            `;

            // Fix: Return standard 1/0 integers to avoid JS string-boolean confusion (e.g. '0' is true)
            const colMetaQuery = `
                SELECT 
                    X.ATTNAME
                    , X.FORMAT_TYPE
                ...
                FROM
                    ${db}.._V_RELATION_COLUMN X
                INNER JOIN
                    ${db}.._V_OBJECT_DATA O ON X.OBJID = O.OBJID
                LEFT JOIN
                    ${db}.._V_RELATION_KEYDATA K 
            ...
            const [dataResult, descResult, colResult] = await Promise.all([
                runQuery(context, `SELECT ROWID, * FROM ${fullTableName} LIMIT 50000`, true, connectionName, connectionManager),
                runQuery(context, metadataQuery, true, connectionName, connectionManager),
                runQuery(context, colMetaQuery, true, connectionName, connectionManager)
            ]);

            // Parse Meta
            let tableComment = '';
            try {
                const descRows = JSON.parse(descResult || '[]');
                if (descRows.length > 0) tableComment = descRows[0].DESCRIPTION;
            } catch (e) { }

            let columnsMeta: any[] = [];
            try {
                columnsMeta = JSON.parse(colResult || '[]');
            } catch (e) { }
            ...
            try {
                ...
                console.log('[EditDataProvider] dataResult type:', typeof raw, 'length:', typeof raw === 'string' ? raw.length : 'N/A');
                console.log('[EditDataProvider] dataResult preview:', typeof raw === 'string' ? raw.substring(0, 200) : 'array');
